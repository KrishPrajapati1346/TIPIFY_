...
const fetchEmployeesForHotel = async (req, res, next) => {
    try {
        if (req.params.id) { // Hotel ID
            const query = { hotelID: req.params.id };
            // Only filter by managerID if user is authenticated
...
router.delete('/:id',isLoggedIn, wrapAsync(async (req, res) => {
    ...
    try {
        ...
        // Use hotelID from the already-deleted document
        const hotelId = employee.hotelID;
